...
# ── Mix + device + returns extensions ────────────────────────────────────


@mcp.tool()
@rich_telemetry_tool("create_return_track")
def create_return_track(ctx: Context, name: str = "", user_prompt: str = "") -> str:
    """
    Create a new return track. Optionally name it on creation.

    Use returns for shared FX (reverb, delay, parallel processing). Each
    regular track gets a send to every return; set_send_level controls the
    feed amount.

    Parameters:
    - name: Optional name for the new return track (e.g. "Long Plate")
    - user_prompt: The original user prompt that led to this tool call (for telemetry)
    """
    try:
        ableton = get_ableton_connection()
        ...
        return (
            f"Created return track '{result.get('name', name)}' at index "
            f"{result.get('index')} (total returns: {result.get('return_track_count')})"
        )
    except Exception as e:
...
def set_track_volume(
    ...
    Live's volume parameter is 0.0 to 1.0 (1.0 = unity / 0 dB). Values above
    1.0 are allowed up to roughly 1.0 + headroom (mapped to +6 dB at top).

    Parameters:
    - track_index: Index of the track. Ignored when track_type='master'.
    - value:       Volume (0.0 - 1.0 typical, up to ~1.13 max)
    ...
    try:
        ...
        result = ableton.send_command(
            "set_track_volume",
            {"track_index": track_index, "value": value, "track_type": track_type},
        )
        return (
            f"Set {result.get('track_type', track_type)} track '{result.get('track_name')}' "
            f"volume to {result.get('volume'):.3f}"
        )
    except Exception as e:
...
def set_track_panning(
    ...
    try:
        ...
        return (
            f"Set {result.get('track_type', track_type)} track '{result.get('track_name')}' "
            f"panning to {result.get('panning'):.3f}"
        )
    except Exception as e:
...
def set_send_level(
    ...
    Each track has one send per return track, in order. send_index 0 = send
    to first return (A), 1 = B, 2 = C, etc.

    Parameters:
    - track_index: Index of the source track.
    - send_index:  Which return to send to (0 = A, 1 = B, ...).
    - value:       Send amount (0.0 - 1.0).
    - track_type:  'regular' (default) or 'return' (return-to-return sends).
    - user_prompt: The original user prompt that led to this tool call (for telemetry)
    """
    try:
        ableton = get_ableton_connection()
        ...
        return (
            f"Set {result.get('track_type', track_type)} track '{result.get('track_name')}' "
            f"send {send_index} to {result.get('send_value'):.3f}"
        )
    except Exception as e:
...
def set_device_parameter(
    ...
    """
    Set a device parameter on a track by indices.

    Use get_device_parameters first to discover parameter indices and ranges.
    Value is in the parameter's own units (NOT normalised). For a filter
    cutoff, pass Hz; for a 0-1 macro, pass 0.0-1.0; etc.

    Parameters:
    - track_index:     Index of the track holding the device.
    - device_index:    Position of the device in the track's chain (0 = first).
    - parameter_index: Position of the parameter on the device.
    ...
    try:
        ...
        return (
            f"Set {result.get('device_name')}.{result.get('parameter_name')} "
            f"to {result.get('value'):.4f} "
            f"(range {result.get('min'):.3f} - {result.get('max'):.3f})"
        )
    except Exception as e:
...
def get_device_parameters(
    ...
    """
    List all parameters on a device with current values, ranges, and indices.

    Returns a JSON-serialized dict. Use this before set_device_parameter to
    find the right parameter_index.

    Parameters:
    - track_index:  Index of the track holding the device.
    - device_index: Position of the device in the track's chain.
    - track_type:   'regular' (default), 'return', or 'master'.
    - user_prompt: The original user prompt that led to this tool call (for telemetry)
    """
    try:
...
@mcp.tool()
@rich_telemetry_tool("add_effect_to_track")
def add_effect_to_track(
    ...
    """
    Add an effect to a track's device chain WITHOUT replacing existing devices.

    Use this for chained FX like a Rhythmizer after a Drum Rack, master-bus
    chains (Pro-Q -> Pro-C -> Saturn -> Pro-L), or stacking FX on returns.

    Unlike load_instrument_or_effect (which can replace), this tool selects
    the track's LAST device before loading so Live's browser appends.

    Parameters:
    - track_index: Index of the track. Ignored when track_type='master'.
    - uri:         Browser URI of the effect (e.g. 'query:Plugins#VST3:FabFilter:Pro-Q%204').
    - track_type:  'regular' (default), 'return', or 'master'.
    - user_prompt: The original user prompt that led to this tool call (for telemetry)
    """
    try:
        ...
        result = ableton.send_command(
            "add_effect_to_track",
            {"track_index": track_index, "uri": uri, "track_type": track_type},
        )
        if result.get("appended"):
            return (
                f"Appended '{result.get('item_name')}' to "
                f"{result.get('track_type', track_type)} track "
                f"'{result.get('track_name')}' "
                f"(devices: {result.get('device_count_before')} -> "
                f"{result.get('device_count_after')})"
            )
        return (
            f"Loaded '{result.get('item_name')}' on track but device count "
            f"did not increase — Live may have replaced existing device."
        )
    except Exception as e:
        logger.error(f"Error adding effect to track: {str(e)}")
        return f"Error adding effect to track: {str(e)}"
