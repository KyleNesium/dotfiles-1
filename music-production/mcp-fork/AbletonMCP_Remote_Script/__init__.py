...
class AbletonMCP(ControlSurface):
    ...
    def _process_command(self, command):
        ...
        try:
            ...
            # Commands that modify Live's state should be scheduled on the main thread
                                 ...
                                 "create_return_track", "set_track_volume", "set_track_panning",
                                 "set_send_level", "set_device_parameter", "get_device_parameters",
                                 "add_effect_to_track"]:
                ...
                def main_thread_task():
                    try:
                        ...
                        # ── Mix + device + returns extensions ─────────────────────
                            ...
                            result = self._set_track_volume(track_index, value, track_type)
                        elif command_type == "set_track_panning":
                            track_index = params.get("track_index", 0)
                            value = params.get("value", 0.0)
                            track_type = params.get("track_type", "regular")
                            result = self._set_track_panning(track_index, value, track_type)
                        elif command_type == "set_send_level":
                            track_index = params.get("track_index", 0)
                            send_index = params.get("send_index", 0)
                            value = params.get("value", 0.0)
                            track_type = params.get("track_type", "regular")
                            result = self._set_send_level(track_index, send_index, value, track_type)
                            ...
                            result = self._set_device_parameter(
                                track_index, device_index, parameter_index, value, track_type)
                        elif command_type == "get_device_parameters":
                            track_index = params.get("track_index", 0)
                            device_index = params.get("device_index", 0)
                            track_type = params.get("track_type", "regular")
                            result = self._get_device_parameters(track_index, device_index, track_type)
                        elif command_type == "add_effect_to_track":
                            track_index = params.get("track_index", 0)
                            uri = params.get("uri", "")
                            track_type = params.get("track_type", "regular")
                            result = self._add_effect_to_track(track_index, uri, track_type)
    ...
    def _resolve_track(self, track_index, track_type):
        """Resolve a track by index and type (regular | return | master)."""
    ...
    def _create_return_track(self, name):
        ...
        try:
            ...
            new_track = return_tracks[new_index]
            if name:
                new_track.name = name
            return {
                "index": new_index,
    ...
    def _set_track_volume(self, track_index, value, track_type):
        """Set track volume (0.0 - 1.0 nominal, can go above 1.0 up to +6dB)."""
        try:
    ...
    def _set_track_panning(self, track_index, value, track_type):
    ...
    def _set_send_level(self, track_index, send_index, value, track_type):
        """Set the send level (0.0 - 1.0) to a return track."""
        try:
            track = self._resolve_track(track_index, track_type)
            if send_index < 0 or send_index >= len(track.mixer_device.sends):
                raise IndexError("Send index out of range")
            track.mixer_device.sends[send_index].value = float(value)
            return {
                "track_type": track_type,
                "track_index": track_index,
                "track_name": track.name,
                "send_index": send_index,
                "send_value": track.mixer_device.sends[send_index].value,
            }
        except Exception as e:
            self.log_message("Error setting send level: " + str(e))
            raise

    def _set_device_parameter(self, track_index, device_index, parameter_index, value, track_type):
        """Set a device parameter by index. Value is in the parameter's own range."""
        try:
            track = self._resolve_track(track_index, track_type)
    ...
    def _get_device_parameters(self, track_index, device_index, track_type):
        """List all parameters on a device with current values + ranges."""
        try:
            track = self._resolve_track(track_index, track_type)
            if device_index < 0 or device_index >= len(track.devices):
                raise IndexError("Device index out of range")
    ...
    def _add_effect_to_track(self, track_index, uri, track_type):
        """Append an effect to a track's device chain (regular | return | master).

        Forces selected_device to the last device so Live's browser.load_item()
        inserts AFTER instead of replacing. Use this for chained effects like
        Rhythmizer Ultra after a Drum Rack, master-bus FX chains, or stacking
        FX on returns.
        """
        try:
            track = self._resolve_track(track_index, track_type)
            ...
            self._song.view.selected_track = track
            # Force selected_device to the LAST device so load_item appends
            if len(track.devices) > 0:
                track.view.selected_device = track.devices[-1]
            ...
            return {
                ...
                "device_count_after": after_count,
                "appended": after_count > before_count,
            }
        except Exception as e:
            self.log_message("Error adding effect to track: " + str(e))
            self.log_message(traceback.format_exc())
            raise
